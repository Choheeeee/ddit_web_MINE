...
/**
 * Model2 아키텍처 : SRP(단일 책임의 원칙)에 따라 request에 대한 처리자와 response 처리자를 분리하는 구조
 *
 * SRP의 구조로 작동하는 MVC패턴 : 하나의 어플리켕션을 구조화할때 역할별로 3개 이상의 레이어를 분리하는 구조
 * 
...
public class FactorialServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(param==null || param.trim().isEmpty() || !param.matches("\\d{1,3}")){
			...
			// 		result *= tmp;
			// 	}
				String expression = MessageFormat.format("{0} != {1}", operand, result);	//Model = Data
		...
		if(status==200) {
			...
			//마샬링 작업 (native언어로 표현된 데이터를 공통된 메세지 교환 형식인 json이나 xml로 바꾸는 작업)
			if(accept.contains("json")) {
				response.setContentType(MediaType.APPLICATION_JSON.getMimeType());
				String expression = (String) request.getAttribute("expression");	//native언어로 표현된 데이터
