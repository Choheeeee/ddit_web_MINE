...
<!-- 모든 요청은 form태그 => 동기요청을 발생시킴 그때 submit이벤트 사용 // 비동기요청으로 하려면 동기요청인 submit이벤트 중단시키고 코드 변환  -->

<form id="calForm" method="post">
   <input type="number" name="year" placeholder="년도" data-init-value="<%=thisMonth.getYear() %>"/>
   <select name="month" data-init-value="<%=thisMonth.getMonthValue() %>">      <!-- init-value=> 키가됨 () -->      
