...
$("[data-pg-role='searchUI']").on("click", '[data-pg-role="searchBtn"]', function(){
	...
	$targetForm.get(0).requestSubmit();	//이 코드에 의해서, 아래에 submit이벤트가 발생함
});
//비동기로 동적으로 생성되는 엘리먼트에 이벤트를 바인딩하면, 만들어지지 않았을 수도 있으므로 undefined가 뜰 수 있음. 그래서 동적으로 생성되는 엘리먼트에 이벤트를 바인딩 하려면 이미 생성돼있는 부모객체 등을 이용하며, 이때 버블링 구조를 활용해야함.
